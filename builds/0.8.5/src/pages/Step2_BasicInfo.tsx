...
const FONT_FAMILY_OPTIONS = [
  ...
  { value: "STLiti", label: "隶书" },
  { value: "STXingkai", label: "行楷" },
...
function Step2_BasicInfo({ onNext, onPrev }: Step2Props) {
  ...
  useEffect(() => {
    if (currentCard) {
      form.setFieldsValue({
        ...
        cardFontFamily: currentCard.cardFontFamily || "Microsoft YaHei",
        cardFontSize: currentCard.cardFontSize ?? 16,
        cardTextColor: currentCard.cardTextColor || "#000000",
      });
    }
  }, [currentCard, form]);
  ...
  const handleValuesChange = (
    ...
    // ColorPicker returns Ant Design Color object, convert to hex string
    const normalized: Record<string, unknown> = { ...allValues };
    if (normalized.cardTextColor && typeof normalized.cardTextColor === "object") {
      const c = normalized.cardTextColor as { toHexString?: () => string };
      if (typeof c.toHexString === "function") {
        normalized.cardTextColor = c.toHexString();
      }
    }
    updateBasicInfo(normalized as never);
  };
  ...
  return (
    <div>
      ...
      >
        ...
        <Divider>卡面样式</Divider>

        <Space style={{ width: "100%" }} size="middle">
          <Form.Item name="cardFontFamily" label="字体" style={{ minWidth: 180 }}>
            <Select options={FONT_FAMILY_OPTIONS} />
          </Form.Item>

          <Form.Item name="cardFontSize" label="字号">
            <InputNumber min={10} max={48} placeholder="16" addonAfter="px" />
          </Form.Item>

          <Form.Item name="cardTextColor" label="文字颜色">
