...
function exportCardsToJSON(cards: ICard[]): string {
  const payload = {
    version: "0.9.0",
...
function exportCardsToMarkdown(cards: ICard[]): string {
  const dateStr = new Date().toISOString().slice(0, 10);
  const lines: string[] = [`# CardMaker 卡牌导出 - ${dateStr}`, ""];

  for (const card of cards) {
    const name = card.displayName || card.name || "未命名卡牌";
    ...
    const typeLabel = card.type
      ...
      : "未知";
    const category = card.categoryPath ?? "";

    lines.push(`## ${name} (${rarityLabel})`);
    lines.push(`- **类型**: ${typeLabel}`);
    lines.push(`- **所属**: ${category}`);
    lines.push("");

    const desc = generateCardDescription(card);
    lines.push(desc);
    lines.push("");
    lines.push("---");
    lines.push("");
...
function batchExportAllCards(cards: ICard[], format: "json" | "md"): void {
  const today = new Date();
  const pad = (n: number) => String(n).padStart(2, "0");
  const dateStamp = `${today.getFullYear()}${pad(today.getMonth() + 1)}${pad(today.getDate())}`;

  if (format === "json") {
    const content = exportCardsToJSON(cards);
    downloadFile(content, `cardmaker_export_${dateStamp}.json`, "application/json");
  } else {
    const content = exportCardsToMarkdown(cards);
    downloadFile(content, `cardmaker_export_${dateStamp}.md`, "text/markdown");
  }
}
